/**
 * Journey Tags Recommended Rule
 *
 * **Why this rule exists:**
 * In Aabha's context engineering framework, **tags** provide categorization and searchability for
 * journeys. Tags help organize journeys by domain, customer segment, business process, or any other
 * meaningful classification, making them easier to discover and manage.
 *
 * Tags enable AI to:
 * 1. **Enable discovery** - Make journeys searchable by category
 * 2. **Organize by domain** - Group journeys by business domain
 * 3. **Filter and query** - Enable filtering by customer segment, process type, or purpose
 * 4. **Generate reports** - Create categorized reports and dashboards
 *
 * Missing tags make it harder to discover, organize, or filter journeys.
 *
 * **What it checks:**
 * - Journeys should have `tags` field (recommended)
 *
 * **Examples:**
 * ```typescript
 * // ✅ Good - Has tags
 ...
 *   tags: ['onboarding', 'account-opening', 'digital', 'customer-facing']
 * })
 *
...
export const journeyTagsRecommended = createRule<[], MessageIds>({
  name: 'journey-tags-recommended',
  meta: {
    type: 'suggestion',
    docs: {
      description:
        'Journeys should have tags field. Tags provide categorization and searchability for journeys.',
    },
    messages: {
      missingTags:
        "Journey '{{name}}' is missing a 'tags' field. Tags provide categorization and searchability for journeys, making them easier to discover and organize. Consider adding tags (e.g., 'tags: [\"onboarding\", \"account-opening\", \"digital\"]').",
    },
    schema: [],
    fixable: 'code',
  },
            ...
            context.report({
              ...
              fix(fixer) {
                ...
                const tagsTemplate = needsComma
                  ? `,\n  tags: [],  // TODO: Add tags for categorization (e.g., ['onboarding', 'account-opening', 'digital'])`
                  : `\n  tags: [],  // TODO: Add tags for categorization (e.g., ['onboarding', 'account-opening', 'digital'])`;
